...
同时启用 `--model` 与 `--verify` 时，默认最多 2 轮修复；可加 `--max-repairs 0` 仅验证首次输出，范围 0..10。代码/断言失败会把最新诊断反馈给模型并重新验收；环境故障、无效模型补丁或模型超时直接停止。每轮仍只允许修改当前指定页面。

`repair-report.json/md` 记录总结果、修复次数与逐轮耗时；`rounds/0` 是首次模型输出，后续数字目录保存修复后的页面快照、模型报告、验证报告和日志。模型报告包含各页前后摘要、用量及耗时；所有页面会逐页复核，修复轮数不等于模型请求次数。最终状态来自工具重跑，不采用模型自述。默认修复预算是有限次数，不保证模型一定修好。
...
首次真实调用已记录在 [模型实验](artifacts/model-static/report.md)：接口与构建通过，但浏览器发现 rpx 样式回退，尚未完成修复。自动 verify 和有限修复已实现；真实模型完整咖啡验收仍待完成。
